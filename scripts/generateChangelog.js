const generateChangeLog = async (releaseDate, targetLang = "fa") => {
  ...
  if (!TRELLO_KEY || !TRELLO_TOKEN || !BOARD_ID) {
    if (!TRELLO_KEY || !TRELLO_TOKEN || !BOARD_ID) {
      throw new Error("Missing TRELLO_KEY, TRELLO_TOKEN, or BOARD_ID in .env");
    }
  ...
  const groupCards = async (cards) => {
    ...
    const translations = await translateMissingSections(uniqueSections, targetLang);
   ...
   return buildOutput(result, releaseDate);
  };

  const buildOutput = (grouped, releaseDate) => {
        ...
        categories: [
          {
            ...
            items: Object.entries(project.feat).map(
              ([title, descriptions]) => ({
                title,
                descriptions,
              })
